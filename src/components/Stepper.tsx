...
import { Bookmark, Sparks, Search } from "iconoir-react"
  ...
  return (
    <section className="w-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] md:bg-[length:800px_400px] bg-[length:300px_200px] bg-no-repeat bg-top from-zinc-400/10 to-60% to-transparent">
      ...
      <div className="flex flex-col items-center gap-4 my-24">
        ...
        <h2 className="max-w-md text-4xl font-semibold text-center text-transparent md:max-w-xl md:text-5xl from-zinc-400 via-zinc-100 to-zinc-400 bg-gradient-to-r bg-clip-text">Manage your bookmarks with just 3 easy steps</h2>
      </div>
        <ul className="grid w-full grid-cols-1 gap-8 sm:grid-cols-3">
          ...
          <ListItem icon={<Sparks className="size-5" />} title="Tag bookmarks" description="Generate smart tags with AI, making it a breeze to find and access your most treasured pages." />
          <ListItem icon={<Search className="size-5" />} title="Filter and search" description="Easily filter your bookmarks by tags or experience lightning-fast searching with the command menu." withDivider={false}  />
        </ul>
      </div>
    </section>
  )
...
const ListItem = ({ icon, title, description, withDivider = true }: ListItemProps) => {
  return (
    <li className="relative flex sm:flex-col sm:items-center sm:text-center group">
      ...
      <div className="flex items-center justify-center duration-200 border rounded-lg size-10 aspect-square bg-zinc-950 border-zinc-700 text-zinc-100 group-hover:scale-110 will-change-transform">
        {icon}
      </div>
