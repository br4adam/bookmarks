...
const BookmarkTags = ({ bookmark }: Props) => {
  ...
  return (
    <div onClick={() => setEditable(true)} ref={tagListRef} className="flex flex-wrap gap-2 mb-2 text-xs">
      { !bookmark.tags.length && !editable && <p className="py-1 text-zinc-500">add tags...</p> }
      { newTags.map((tag, index) => (
        <span key={tag} className={`px-2 py-[2px] h-6 flex items-center truncate rounded-md border border-zinc-800 text-zinc-400 cursor-default ${editable ? "gap-1" : "gap-0"}`}>
          { tag } 
