...
import { GitHub, MagicWand } from "iconoir-react"
          ...
          <div className="flex items-center justify-center min-h-full p-4">
            <Transition.Child as={Fragment} enter="ease-out duration-200" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
              <Dialog.Panel className="w-full max-w-md p-6 overflow-hidden text-left align-middle transition-all transform rounded-lg bg-zinc-200">
                <Dialog.Title as="h3" className="text-lg font-medium text-zinc-900">Welcome back</Dialog.Title>
                ...
                <div className="flex flex-col items-center gap-2 mt-4 text-zinc-900">
                  <form className="flex flex-col items-center w-full gap-2" onSubmit={handleLogin}>
                    <input className="w-full px-4 py-2 mt-1 bg-transparent border rounded-md border-zinc-700 focus:border-zinc-500 focus:outline-none" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" autoFocus />
                    <Button type="submit" className="justify-center w-full bg-zinc-900 text-zinc-200 hover:bg-zinc-800"><MagicWand width={18} strokeWidth={1.75} />Send Magic Link</Button>
                  </form>
